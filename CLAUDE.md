...
│   └── og-card.png              ← 1200x630 social card, the og:image every link
│                                   preview shows. Pale sage ground, ink-green type,
│                                   and the REAL 2025 reviewer-score distribution as
│                                   its chart. TRACKED SOURCE, not generated by the
│                                   build: it renders text, and a font missing on a
│                                   CI runner would silently draw it in a fallback
│                                   face with nobody looking. Regenerate by hand with
│                                   analysis/make_og_card.py when the headline counts
│                                   change, i.e. when a year is added
├── scraper/
...
│   ├── seo.py                   ← the page table, sitemap.xml, robots.txt, llms.txt
│   │                               and the schema.org JSON-LD. ONE list of Page
│   │                               records drives a page's <head>, its sitemap <loc>
│   │                               and the llms.txt entry, so the three cannot drift.
│   │                               Called from build_site.py; stdlib only
│   ├── make_og_card.py          ← draws static/og-card.png. Run BY HAND, not by the
│   │                               build; see static/ above for why
...
│   ├── test_seo.py              ← the crawler-facing output, which is invisible: a
│   │                               wrong canonical URL, a sitemap that quietly stopped
│   │                               listing a year and a broken JSON-LD block all
│   │                               render identically in a browser. Checks the built
│   │                               files on disk, never the table that made them
...
│   ├── sitemap.xml              ← all 10 pages; written by analysis/seo.py
│   ├── robots.txt               ← allows everything, names the sitemap, names the AI
│   │                               crawlers explicitly. Disallows NOTHING: /charts/
│   │                               must stay fetchable or the pages that embed it
│   │                               look empty to a renderer
│   ├── llms.txt                 ← site map for language models, incl. the four
│   │                               caveats a summariser drops
│   ├── year/
│   │   └── YYYY.html
...
# 5. Generate HTML pages (also writes sitemap.xml, robots.txt and llms.txt)
python analysis/build_site.py

# 5b. Only when a year is added: redraw the social card that link previews show.
#     By hand, not part of the build; see "Search Engines, Social Cards and LLMs".
...
- **`_BUNDLE_SRI` pins the partial bundles' hashes per plotly.js version**,
  ...
  of breaking a local preview. Restore it with
  `curl -sL <url> | openssl dgst -sha256 -binary | openssl base64 -A`.

  **The table must hold every version any environment builds with, not just the
  newest, and `deploy.yml` sets `MICCAI_STRICT_SRI=1` so a deploy refuses
  rather than degrades.** Both rules exist because the first four releases
  shipped with no integrity attribute on 39 of 63 chart files. The table held
  `3.7.0` alone, which is what the maintainer's conda environment vendors,
  while `requirements-ci.txt` pins `plotly==6.8.0`, which vendors **3.6.0**.
  Every deployed build therefore took the degrade path; the 18 full-bundle
  charts kept plotly's own hash, so the site looked consistent. Nothing caught
  it: the warning scrolls past in a CI log, and a page works perfectly without
  SRI. Found 2026-09-16 when a build in `.venv` printed the warning locally.

  **plotly.py and plotly.js version numbers are different.** Check which .js a
  given .py vendors before adding a row:
  `python -c "import plotly.offline as o; print(o.get_plotlyjs_version())"`
  (plotly.py 6.8.0 vendors 3.6.0; 6.9.0 vendors 3.7.0).

After any change here, check the assignment with
`grep -ho 'cdn\.plot\.ly/plotly[^"]*' website/charts/*.html | sort | uniq -c`;
today that is 34 basic, 5 gl2d, 18 full, with the six d3 co-authorship files
loading no plotly at all. The version in those URLs is whatever the building
environment vendors, so it differs between a conda build and a CI build until
the two are pinned together; the counts do not.
...
## Search Engines, Social Cards and LLMs

Built by `analysis/seo.py`, called from `build_site.py`. Covered by
`tests/test_seo.py`.

**One page table drives everything.** `seo.collect_pages()` returns a `Page`
per indexable page, and that same record becomes the `<title>`, the
`<link rel="canonical">` and the `<meta name="description">` in the page head,
the `<loc>` in `sitemap.xml`, and the entry in `llms.txt`. There is no second
list to keep in step. A template rendered without a `page` raises in Jinja
rather than writing an empty canonical, which is worse than none.

Until 2026-09-16 all ten pages shared one hardcoded description and one
`og:title`, so every page told a search engine it was the same page as the
other nine.

| Written into `website/` | What it is |
|---|---|
| `sitemap.xml` | `<loc>` + `<lastmod>` only. `<changefreq>` and `<priority>` are in the schema but Google ignores both, and a priority every page shares says nothing |
| `robots.txt` | Allows everything, names the sitemap absolutely, and names 12 AI crawlers explicitly. The named blocks change no behaviour (the wildcard already covers them); they are there because blocking that list is now a common default and silence reads as an oversight |
| `llms.txt` | A proposed convention, not a standard, and no provider has said it reads one. It earns its place anyway as the one page stating what the data **cannot** support |
| `assets/og-card.png` | Copied from `static/`; see below |

**`<meta name="keywords">` is deliberately absent.** Google dropped it as a
signal in 2009; Bing treats a stuffed one as spam. The keywords that are read
live in the title, the description, the headings, and the `keywords` array of
the JSON-LD `Dataset`.

**The JSON-LD is a three-node `@graph`: `WebSite`, `Person`, `Dataset`.**
`Dataset` is the one that matters: it is what Google Dataset Search indexes,
and a researcher is likelier to search for "MICCAI peer review data" than for
this site by name. `Person` makes the attribution machine-readable rather than
only printed in the footer, and `Dataset.creator` points at it. The graph is
serialized with `<` escaped as `\u003c`, because it is written between
`<script>` tags in a template with autoescape off.

**Chart files carry `noindex, follow`.** The 63 files in `website/charts/` are
iframe fragments: measured on the built output, one holds **zero characters of
static text**, since the whole figure is a JSON blob inside a `<script>`. No
heading, no prose, no navigation, no link back. Indexed, they compete for the
same queries as the parent page that has the caption, and a reader landing on
one sees a chart with no way to learn what it is of. To link to a single plot,
use the section anchor on its parent page (`#sec-map`, `#sec-reviews`,
`#sec-network`); that lands a reader on the chart **with** its explanation.
`follow` rather than plain `noindex` keeps the click-through URLs on the map
points discoverable. This is a judgement call, so it is a switch:
`chart_settings.index_chart_pages`, default `false`.

**`robots.txt` disallows nothing, and must not.** The chart files are the
content of every analysis page; a renderer blocked from fetching them sees
those pages as nearly empty, and a crawler has to be allowed to read a file in
order to obey the `noindex` inside it. Those two rules work together and
breaking either alone silently undoes the pair.

**The social card draws the real data.** `make_og_card.py` reads
`data/processed/miccai_all.json` and plots the actual distribution of reviewer
scores for `CHART_YEAR` (2025, the most recent complete year). That is the
subject of the whole project, so the card shows it rather than an abstract
texture; six bars also survive the ~500px a timeline renders a card at, where
fine texture turns to mush. An earlier draft drew 320 random dots and they read
as dust. `_fit()` asserts every string fits the safe width and raises if not: a
clipped card exits 0 and looks plausible, and the overflow only shows up once
the link is posted.

**The social card is a tracked source asset, not a build step.** This is the
opposite of the year logos, which `logo_assets.py` converts at build time, and
the reason is fonts: the card renders text, so generating it in CI would depend
on a typeface being installed on the runner. A missing one would not fail the
build, it would draw the card in a fallback face, and nobody inspects a social
card after a deploy. `python analysis/make_og_card.py` writes
`static/og-card.png`; review the PNG in the diff. Re-run it when a year is
added. Nothing breaks if it is never re-run; the card just quotes older counts
and an older year's chart.

**Its faces are resolved by family name through `fc-match`, never by path**,
and the script verifies both the family and the exact numeric weight that come
back. Each check catches a different silent substitution: `fc-match` never
fails, so an uninstalled family returns some unrelated default, and it also
answers with the nearest weight it has while reporting a style string
("Medium,Regular") that a style-name check would accept as Regular. Resolving
by path would also put a home directory into the repository, which
`make_release.sh` greps for and refuses to publish.

**`site_settings.site_url` is the one config key that carries a real hazard.**
It is written into `<link rel="canonical">` on every page, which is a statement
that the page it sits on is a copy of the page at that address. A fork that
published without changing it would be telling Google that all of its own pages
are duplicates of this site's. Unlike a wrong footer link, that is not repaired
by editing it later: the pages have to be recrawled and reassessed first.
